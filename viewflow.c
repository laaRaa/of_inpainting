...
#ifdef ENABLE_OVERLINES
#include "drawsegment.c"
#include "marching_squares.c"
#endif//ENABLE_OVERLINES


#include "smapa.h" // "smart" parameters (read from env)
...
static
void viewflow_middlebury(uint8_t *py, float *px, int w, int h, float range)
{
	float (*x)[w][2] = (void*)px;
	uint8_t (*y)[w][3] = (void*)py;
	if (!isfinite(range)) {
...
int main_viewflow(int c, char *v[])
{
	...
	if (pd != 2)
		return fprintf(stderr,
				"ERROR(%s): input is not a vector field", *v);
	...
	if (isfinite(satscale)) {
		...
		viewflow_middlebury(view[0][0], flow[0][0], w, h, MRANGE());
#else
	if (satscale >= 0)
		viewflow_flat(view[0][0], flow[0][0], w, h, satscale);
	else
		viewflow_middlebury(view[0][0], flow[0][0], w, h, -satscale);
#endif//ENABLE_OVERLINES
